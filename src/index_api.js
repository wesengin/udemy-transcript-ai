...
/**
 * Waits for the first selector in the list to appear.
 * Tries each selector sequentially with a shorter timeout to stay responsive.
 */
async function waitForFirstSelector(page, selectors, options = {}) {
  const { timeoutPerSelector = 5000, ...waitOptions } = options;

  for (const selector of selectors) {
    try {
      await page.waitForSelector(selector, { ...waitOptions, timeout: timeoutPerSelector });
      return selector;
    } catch (err) {
      if (
        err &&
        (err.name === 'TimeoutError' ||
          (err.message && err.message.toLowerCase().includes('exceeded')))
      ) {
        continue;
      }
      throw err;
  ...
  throw new Error(`Unable to locate any selector from: ${selectors.join(', ')}`);
}
...
async function main() {
  ...
  if (process.argv.length < 3) {
    console.error('Please provide course URL, downloadSrt (true/false), and tabCount');
    console.error('Example: node src/index_api.js https://www.udemy.com/course/your-course-name false 5');
    process.exit(1);
  }

  let courseUrl = process.argv[2];
  const downloadSrt = process.argv[3] === 'true';
  const tabCount = parseInt(process.argv[4]) || 5;
  ...
  console.log(`Download SRT: ${downloadSrt}`);
  ...
  try {
    ...
    const emailSelector = await waitForFirstSelector(page, emailSelectorCandidates, {
      timeoutPerSelector: 8000,
      visible: true
    });
    ...
    await page.type(emailSelector, process.env.UDEMY_EMAIL, { delay: 100 });
    ...
    const curriculumItems = await fetchAllCurriculumItems(page, apiUrl);

    // Process course structure
    console.log('Processing course structure...');
    const courseStructure = processCourseStructure(curriculumItems);
...
// Helpers to fetch course curriculum
async function fetchJsonWithRetries(page, url, label, maxAttempts = 3) {
  let lastError = null;

  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    console.log(`Attempt ${attempt} to fetch ${label}...`);
    try {
      ...
      if (!parsed || typeof parsed !== 'object') {
        throw new Error('JSON parsed but response is empty');
      }

      return parsed;
    } catch (err) {
      lastError = err;
      console.warn(`[Attempt ${attempt}] Failed to fetch ${label}: ${err.message}`);
  ...
  throw new Error(`Could not retrieve ${label}. Make sure you have access to this course and try again.`);
}

async function fetchAllCurriculumItems(page, initialUrl) {
  const aggregated = [];
  ...
  while (url) {
    if (visitedUrls.has(url)) {
      console.warn(`Duplicate curriculum page detected (${url}). Stopping to avoid an infinite loop.`);
      break;
    }

    visitedUrls.add(url);
    const label = `course content page ${pageIndex}`;
    const json = await fetchJsonWithRetries(page, url, label);

    if (!json.results || !Array.isArray(json.results)) {
      throw new Error('Course content response did not include a results list.');
    }

    aggregated.push(...json.results);

    if (json.next) {
      url = new URL(json.next, url).toString();
      pageIndex++;
    } else {
      url = null;
    }
  ...
  console.log(`Collected ${aggregated.length} curriculum items across ${visitedUrls.size} page${visitedUrls.size === 1 ? '' : 's'}.`);
  return aggregated;
}
